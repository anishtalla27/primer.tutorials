...
```{r wisdom-5}
question_text(NULL,
	message = "For a given person, assume that the treatment variable could be “received a phone call” or “did not receive a phone call.” If the person receives a phone call, their voting behavior might be “voting.” If the person does not receive a phone call, their behavior might be “not voting.” The causal effect is the difference between these two potential outcomes--—“voting” minus “not voting”—--even though this difference does not have a numeric value. That difference is still the causal effect, even if we cannot assign a specific number to it. In many cases, we choose to assign numbers, such as 1 for voting and 0 for not voting. This lets us report a numeric causal effect, but it is important to remember that these numbers are just a coding choice.",
	answer(NULL, correct = TRUE),
	allow_retry = FALSE,
	incorrect = NULL,
	rows = 6)
...
This problem is causal so one of the covariates is a treatment. In our problem, the treatment is whether a perso received a particular postcard (the `treatment` variable). There is a potential outcome for each of the possible values of the treatment --—receiving each type of postcard or receiving no postcard.
...
Change the call to `fit()` to `fit(factor(primary_06) ~ treatment, data = shaming))`.
